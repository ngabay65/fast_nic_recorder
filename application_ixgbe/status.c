...
int is_magic_ok (struct global_share_struct *gshare)
{
  if (gshare->magic1 == MAGIC1 && gshare->magic2 == MAGIC2)
    return 1;
  printf ("magic1=%08x  magic2=%08x\n", gshare->magic1, gshare->magic2);
  return 0;
}
...
int main (int argc, char *argv[])
{
    ...
    // user_issue_start 
    printf ("user_issue_start=%d\n", gshare->data [USER_ISSUE_START]);
    printf ("user_issue_stop=%d\n", gshare->data [USER_ISSUE_STOP]);
