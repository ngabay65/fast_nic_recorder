...
void dump_one_packet (unsigned char *data, int len)
{
  rec_hdr.incl_len = rec_hdr.orig_len = len;
  write (f_pcap, &rec_hdr, sizeof (rec_hdr));
  write (f_pcap, data, len);
}
...
void dump_packet (unsigned char *data, struct bucket_info_struct *bucket_info, long long base, long long start, long long count, long long *processed)
{
  ...
  *processed = 0;
  ...
  while (1) {
    ...
    count--;
    (*processed)++;
    dump_one_packet (data + 240 * 1024 * cur_partial + local_len [cur_partial], bucket_info->len [cur_row][cur_partial]);
...
{
    int fout;
    int file_count, writer_num = 0;
    char fname [256];
    int fd;
    int cur;
    long long offs, start = 0, count, processed, total_packets = 0;
    unsigned char *data_ptr;

    if (argc != 4) {
      printf ("block2libpcap <start_block#> <count> <pcap_fname>\n");
      return 0;
    }
    ...
    f_pcap = open (argv [3], O_WRONLY | O_CREAT);
    if (f_pcap == -1) {
        perror ("open");
        return 1;
    }
    write (f_pcap, &hdr, sizeof (hdr));
    rec_hdr.ts_sec = 0;
    rec_hdr.ts_usec = 0;
