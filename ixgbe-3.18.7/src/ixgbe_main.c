			     ...
			     struct ixgbe_ring *ring)
{
	...
	ring->head = hw->hw_addr + IXGBE_RDH(reg_idx);
...
printk ("rx_buf_len = %d\n", rx_buf_len);
if (adapter->flags2 & IXGBE_FLAG2_RSC_ENABLED)
  printk ("IXGBE_FLAG2_RSC_ENABLED enabled\n");
else
  printk ("IXGBE_FLAG2_RSC_ENABLED disabled\n");
...
static int ixgbe_setup_all_rx_resources(struct ixgbe_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		err = ixgbe_setup_rx_resources(adapter->rx_ring[i]);
                if (pbm_adapter == adapter && i == 0)
                  pbm_rx_ring = adapter->rx_ring [0];
		if (!err) {
			continue;
		}
...
#endif
	e_info(drv, "[%s]NIC Link is Up %s, Flow Control: %s\n",
		(netdev == pbm_netdev) ? "nlsim" : "xxxxx",
	       (link_speed == IXGBE_LINK_SPEED_10GB_FULL ?
	       "10 Gbps" :
				 ...
				 const struct pci_device_id *ent)
{
	...
	adapter->msg_enable = (1 << DEFAULT_DEBUG_LEVEL_SHIFT) - 1;
        ...
        if (pbm_dev_count == NLSIM_PORT_USE) {
          // use one port only
          pbm_pdev = pdev;
          pbm_netdev = netdev;
          pbm_adapter = adapter;
